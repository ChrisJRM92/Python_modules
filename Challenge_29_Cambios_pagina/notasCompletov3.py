...
def fnBuscar():
    ...
    if estudianteEncontrado == None:
        ...
        partesEstudiante=estudianteEncontrado
        limpiar()
...
def fnGuardar():
    ...
    inasistencias = fnObtenerEntero(
        txtInasistencias,
        lblErrorInasistencias,
        0
    )
    if (cedula is None or nombre is None or apellido is None or
        nota1 is None or nota2 is None or nota3 is None or
        inasistencias is None):
        return

    total = calcularTotal(nota1, nota2, nota3, inasistencias)
    lblTotal.config(text=f"{total:.2f}")
    estEncontrado = buscarEstudiante(cedula, estudiantes)

    if estEncontrado is not None:
        messagebox.showwarning(
            title="IMPORTANTE",
            message=f"Ya existe el estudiante con la cédula {cedula}"
        )
        return
    estudiantes.append(
        f"{cedula}#{nombre}#{apellido}#{nota1}#{nota2}#{nota3}#{inasistencias}#{total}"
    )
